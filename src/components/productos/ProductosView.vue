  ...
  methods: {
    ...
    async cargarProductos() {
      try {
        ...
        if (response.status === 204) {
          ...
          this.mostrarMensaje(
            "⚠️ No se encontraron productos en el sistema.",
            "warning",
          );
          return;
        }
        ...
        this.mostrarMensaje(
          `✅ ${this.productos.length} producto${this.productos.length === 1 ? "" : "s"} cargado${this.productos.length === 1 ? "" : "s"} correctamente.`,
          "success",
        );
      } catch (error) {
    ...
    async filtrarProductos() {
      ...
      if (!termino || !this.tipoBusqueda) {
        this.mostrarMensaje(
          "⚠️ Por favor, seleccione un tipo de busqueda y un termino.",
          "error",
        );
        return;
      }
      ...
      try {
        ...
        if (response.status === 204) {
          this.productosFiltrados = [];
          this.mostrarMensaje(
            `❌ No se encontraron productos con ${this.tipoBusqueda}: ${termino}`,
            "warning",
          );
          return;
        }
        ...
        if (response.data) {
          ...
          this.mostrarMensaje(
            `✅ Producto${Array.isArray(response.data) && response.data.length > 1 ? "s" : ""} encontrado${Array.isArray(response.data) && response.data.length > 1 ? "s" : ""} correctamente.`,
            "success",
          );
          return;
        }
        ...
        this.productosFiltrados = [];
        this.mostrarMensaje(
          `❌ No se encontró producto con ${this.tipoBusqueda}: ${termino}`,
          "warning",
        );
      } catch (error) {
    ...
    async filtrarProductosPorFecha() {
      try {
        ...
        if (response.data) {
          this.productosFiltrados = Array.isArray(response.data)
            ? response.data
            : [response.data];
          this.mostrarMensaje(
            "✅ " +
              this.productosFiltrados.length +
              " producto(s) encontrado(s) en el rango de fechas.",
            "success",
          );
        }
      } catch (error) {
    ...
    async cargarProveedores() {
      try {
        ...
        if (error.response && error.response.data) {
          ...
          this.mostrarMensaje(
            "Error al conectarse con el servidor de proveedores.",
            "error",
          );
        }
      }
