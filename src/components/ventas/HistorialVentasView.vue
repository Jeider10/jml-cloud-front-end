    ...
    <div :class="['ventas-container', { expanded: menuOpen }]">
      ...
      <div class="form-filtro no-print">
        ...
        <div style="display: flex; gap: 4px;">
          ...
          <input v-else-if="tipoBusqueda !== 'fechaCreacion'"
                 v-model="busqueda"
                 type="text"
          ...
          <label v-if="isAdmin" style="display: flex; align-items: center; gap: 4px; margin-left: 8px; font-size: 13px; cursor: pointer;">
            <input type="checkbox" v-model="filtrarPorMiUsuario" @change="aplicarFiltroUsuario" />
            Ver solo mis ventas
          </label>
        </div>
      </div>
      ...
      <table class="ventas-table">
        ...
        <tr v-for="(venta, idx) in ventasFiltradas" :key="idx">
          ...
          <td>
              <span v-if="venta.estado === 'PENDIENTE'" style="color: #e67e22; font-weight: bold;">
                {{ formatPrecioCOP(venta.valorRecibido || 0) }}
              </span>
            <span v-else>-</span>
          </td>
          <td>
              <span :style="{ color: venta.estado === 'PENDIENTE' ? '#e67e22' : venta.estado === 'CERRADA' ? '#27ae60' : venta.estado === 'ABIERTA' ? '#0077b6' : '#333', fontWeight: 'bold' }">
                {{ venta.estado }}
              </span>
          </td>
          <td>
            ...
            <span v-else-if="venta.comentario && venta.estado === 'CERRADA'" style="font-size: 11px;">
                📝 {{ venta.comentario }} <br><span style="color: #27ae60; font-weight: bold;">✅ PAGADO</span>
              </span>
            <span v-else>-</span>
          </td>
      ...
      <div class="footer-ventas">
        ...
        <span v-if="hayPendientes" class="total" style="color: #e67e22; margin-left: 20px;">⏳ Abonado (pendientes): {{ totalAbonado }}</span>
        <span v-if="hayPendientes" class="total" style="color: #c0392b; margin-left: 20px;">❗ Por cobrar: {{ totalPendiente }}</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    totalPendiente() {
      const total = this.ventasFiltradas
          .filter(v => v.estado === 'PENDIENTE')
          .reduce((acc, v) => acc + (this.totalVenta(v) - (v.valorRecibido || 0)), 0)
  ...
  methods: {
    ...
    async buscarTodasLasOrdenes() {
      try {
        ...
        let ordenes = (response.data || []).map(o => ({
          ...
          total: this.calcularTotalDetalles(o.detalles || []) - (o.descuentoAplicado || 0),
        ...
        if (rolesUsuario.includes(roleName) && session?.user?.identificacion) {
          ...
          ordenes = ordenes.filter(o => Number(o.identificacionVendedor) === miId)
        }
    ...
    async filtrarVentas() {
      ...
      try {
        ...
        if (response.status === 204 || !response.data) {
          this.ventasFiltradas = []
          this.mostrarMensaje('⚠️ No se encontraron ventas con ese filtro.', 'warning')
          return
        }

        // Mapear respuesta
        const ordenes = Array.isArray(response.data) ? response.data : [response.data]
        ...
        if (this.filtrarPorMiUsuario) {
          ...
          if (session?.user?.identificacion) {
            const miId = Number(session.user.identificacion)
            this.ventasFiltradas = this.ventasFiltradas.filter(v => Number(v.identificacionVendedor) === miId)
          }
        }
        ...
        this.mostrarMensaje(`✅ ${this.ventasFiltradas.length} venta(s) encontrada(s).`, 'success')

      } catch (error) {
        console.error('❌ Error al filtrar ventas:', error)
        this.ventasFiltradas = []
        this.mostrarMensaje('❌ Error al buscar ventas en el servidor.', 'error')
      }
    },
    ...
    async filtrarVentasPorFecha() {
      try {
        ...
        if (!response.data || (Array.isArray(response.data) && response.data.length === 0) || response.status === 204) {
          this.ventasFiltradas = []
          this.mostrarMensaje('❌ No se encontraron ventas en el rango de fechas seleccionado.', 'warning')
          return
        }
        ...
        if (rolesUsuario.includes(roleName) && session?.user?.identificacion) {
          const miId = Number(session.user.identificacion)
          ventasMapeadas = ventasMapeadas.filter(o => Number(o.identificacionVendedor) === miId)
        }
        ...
        this.mostrarMensaje('✅ ' + this.ventasFiltradas.length + ' venta(s) encontrada(s) en el rango de fechas.', 'success')

      } catch (error) {
        console.error('❌ Error al filtrar ventas por fecha:', error)
        this.mostrarMensaje(error.message || 'Error al filtrar ventas por fecha', 'error')
      }
    },
    ...
    imprimirHistorial() {
      ...
      ventas.forEach((v, idx) => {
        const productos = (v.productos || []).map(p => p.producto + ' (x' + p.cantidad + ')').join('<br>')
        const precios = (v.productos || []).map(p => this.formatPrecioCOP(p.precio)).join('<br>')
        const subtotales = (v.productos || []).map(p => this.formatPrecioCOP(p.precio * p.cantidad)).join('<br>')
        const total = this.totalVenta(v)

        // 🔹 Descuento formateado
        let descuentoTexto = '-'
        if (v.descuentoAplicado > 0) {
          descuentoTexto = (v.descuentoTipo === 'PORCENTAJE' ? v.descuentoValor + '% ' : '') + '-' + this.formatPrecioCOP(v.descuentoAplicado)
        }

        filas += '<tr>' +
            '<td>' + (idx + 1) + '</td>' +
            '<td>' + (v.cliente || '') + '</td>' +
            '<td>' + (v.identificacionCliente || '') + '</td>' +
            '<td>' + productos + '</td>' +
            '<td>' + precios + '</td>' +
            '<td>' + subtotales + '</td>' +
            '<td>' + descuentoTexto + '</td>' +
            '<td><strong>' + this.formatPrecioCOP(total) + '</strong></td>' +
            '<td>' + (v.estado === 'PENDIENTE' ? this.formatPrecioCOP(v.valorRecibido || 0) : '-') + '</td>' +
            '<td>' + (v.estado || '') + '</td>' +
            '<td>' + (v.comentario || '-') + '</td>' +
            '<td>' + (v.vendedor || '') + '</td>' +
            '<td>' + (v.identificacionVendedor || '') + '</td>' +
            '<td>' + (v.numeroFactura || '') + '</td>' +
            '<td>' + (v.fecha || '') + '</td>' +
            '</tr>'
      })

      const totalGeneral = ventas.filter(v => v.estado === 'CERRADA').reduce((acc, v) => acc + this.totalVenta(v), 0)

      const html = '<!DOCTYPE html><html><head><title>Historial de Ventas</title>' +
          '<style>' +
          ...
          '<div class="footer">Total de Ventas: ' + this.formatPrecioCOP(totalGeneral) + '</div>' +
          '</body></html>'
