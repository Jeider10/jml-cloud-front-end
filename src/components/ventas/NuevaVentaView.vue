  ...
  <div class="nueva-venta-wrapper">
    ...
    <div :class="['venta-container', { expanded: menuOpen }]">
        ...
        <div v-if="modalEliminarOrden.visible" class="modal-overlay">
          <div class="modal-content">
            ...
            <div class="modal-buttons">
              <!-- ✅ Sí -->
              <button class="btn-yes" @click="confirmarEliminarOrden">
                Sí
              </button>
          ...
          <div class="modal-content" style="min-width: 420px; max-width: 520px">
            ...
            <div style="margin: 10px 0; text-align: left; font-size: 14px">
              ...
              <p>
                ...
                {{ formatPrecioCOP(calcularTotalFinal) }}
              </p>
              <p v-if="descuentoAplicado > 0">
                <strong>Descuento:</strong> -{{
                  formatPrecioCOP(descuentoAplicado)
                }}
              </p>
            </div>
            ...
            >
              ...
              <div style="display: flex; flex-direction: column; gap: 8px">
                ...
                <div style="display: flex; align-items: center; gap: 8px">
                  <label style="width: 140px; font-size: 13px"
                    >💵 Efectivo:</label
                  >
                  <input
                    v-model.number="pagosMixtos.efectivo"
                    type="number"
                    min="0"
                    placeholder="0"
                    style="
                ...
                <div style="display: flex; align-items: center; gap: 8px">
                  <label style="width: 140px; font-size: 13px"
                    >💳 Tarjeta Débito:</label
                  >
                  <input
                    v-model.number="pagosMixtos.tarjetaDebito"
                    type="number"
                    min="0"
                    placeholder="0"
                    style="
                ...
                <div style="display: flex; align-items: center; gap: 8px">
                  <label style="width: 140px; font-size: 13px"
                    >💳 Tarjeta Crédito:</label
                  >
                  <input
                    v-model.number="pagosMixtos.tarjetaCredito"
                    type="number"
                    min="0"
                    placeholder="0"
                    style="
                ...
                <div style="display: flex; align-items: center; gap: 8px">
                  <label style="width: 140px; font-size: 13px"
                    >📱 Transferencia/Nequi:</label
                  >
                  <input
                    v-model.number="pagosMixtos.transferencia"
                    type="number"
                    min="0"
                    placeholder="0"
                    style="
                ...
                <div style="display: flex; align-items: center; gap: 8px">
                  ...
                  >
                  <input
                    v-model="pagosMixtos.referencia"
                    type="text"
                    placeholder="Num. aprobación (opcional)"
                    style="
                      flex: 1;
                      padding: 5px;
                      border-radius: 4px;
              ...
              >
                <p>
                  ...
                  {{ formatPrecioCOP(totalPagosMixtos) }}
                </p>
                <p>
                  <strong>Faltante:</strong>
                  {{
                    totalPagosMixtos >= calcularTotalFinal
                      ? formatPrecioCOP(0)
                      : formatPrecioCOP(calcularTotalFinal - totalPagosMixtos)
                  }}
                </p>
                <p>
                  <strong>Cambio (efectivo):</strong>
                  {{
                    formatPrecioCOP(
                      Math.max(totalPagosMixtos - calcularTotalFinal, 0),
                    )
                  }}
                </p>
              </div>
              ...
              <div
                v-if="
                  totalPagosMixtos > 0 && totalPagosMixtos < calcularTotalFinal
                "
                style="
                  margin-top: 8px;
                  padding: 8px;
                ...
                "
              >
                ⚠️ <strong>Pago insuficiente.</strong> Faltan
                {{ formatPrecioCOP(calcularTotalFinal - totalPagosMixtos) }}.
                Puede cerrar la venta con un comentario explicando la deuda.
              </div>

              <!-- 📝 Comentario / Observación — solo habilitado si pago insuficiente -->
              <div style="margin-top: 12px">
                <label style="font-size: 13px; font-weight: bold"
                  >📝 Comentario (requerido si pago insuficiente):</label
                >
                <textarea
                  ...
                  :placeholder="
                    totalPagosMixtos < calcularTotalFinal
                      ? 'Ej: Debe ' +
                        formatPrecioCOP(calcularTotalFinal - totalPagosMixtos) +
                        ', paga el viernes'
                      : 'Sin observaciones (pago completo)'
                  "
                  style="
                    width: 100%;
                    padding: 6px;
            ...
            <div class="modal-buttons" style="margin-top: 15px">
              ...
              <button class="btn-no" @click="modalCerrarVenta = false">
                Cancelar
              </button>
            </div>
          </div>
        ...
        >
          ...
          >
            <p style="font-size: 16px; margin-bottom: 12px">
              ⚠️ <strong>Valor insuficiente</strong>
            </p>
            <p style="font-size: 13px; color: #555">
              El pago ingresado es menor al total a pagar.<br />
              Debe agregar un <strong>comentario</strong> para el historial de
              la deuda.
            </p>
            <div class="modal-buttons" style="margin-top: 15px">
          ...
          <div class="modal-content" style="max-width: 420px">
            ...
            <div v-if="pagoInsuficiente" style="text-align: center">
              <p style="font-size: 16px; margin-bottom: 10px">
                ⚠️ <strong>Pago insuficiente</strong>
              </p>
              <p style="font-size: 13px; color: #555">
                El valor recibido ({{ formatPrecioCOP(totalPagosMixtos) }}) es
                menor al total a pagar ({{
                  formatPrecioCOP(calcularTotalFinal)
                }}).
              </p>
            ...
            <div v-else style="text-align: center">
              <p style="font-size: 16px; margin-bottom: 10px">
                ✅ <strong>Confirmar cierre de venta</strong>
              </p>
              <p style="font-size: 13px; color: #555">
                Total a pagar:
                <strong>{{ formatPrecioCOP(calcularTotalFinal) }}</strong>
              </p>
              <p style="font-size: 13px; color: #555">
                Recibido:
                <strong>{{ formatPrecioCOP(totalPagosMixtos) }}</strong>
              </p>
              <p
                style="font-size: 13px; color: #555"
                v-if="totalPagosMixtos > calcularTotalFinal"
              >
                Cambio:
                <strong>{{
                  formatPrecioCOP(totalPagosMixtos - calcularTotalFinal)
                }}</strong>
              </p>
              <p style="font-size: 13px; margin-top: 12px; font-weight: bold">
      ...
      <div class="form-container no-print">
        <div class="form-row">
          ...
          <input
            :value="
              precioEditando ? venta.precio : formatPrecioInput(venta.precio)
            "
        ...
        >
          ...
          <!-- Select de estados -->
          <select v-model="filtroEstado" id="filtroEstado">
        ...
        <div v-if="ordenesFiltradas.length > 0" class="form-row">
          ...
          <select
            ...
            id="ordenSeleccionada"
            @change="cargarItemsOrdenSeleccionada"
          >
            ...
            >
              {{ orden.nombreCliente || "CONSUMIDOR FINAL" }} |
              {{ orden.estadoOrden }} | Total:
              {{ formatPrecioCOP(totalOrden(orden))
              }}{{
                orden.estadoOrden === "PENDIENTE" && orden.comentario
                  ? " | 📝 " + orden.comentario
                  : ""
              }}
            </option>
        ...
        <tbody>
          ...
          <tr v-for="(item, idx) in items" :key="'item-' + idx">
            ...
            <td class="cantidad-cell">
              ...
              <div
                class="mini-controls no-print"
                v-if="ordenEstado === 'ABIERTA'"
              >
                <input
            ...
            <td
              v-if="ordenEstado === 'PENDIENTE' && idx === 0"
              :rowspan="items.length"
              style="vertical-align: middle; font-weight: bold; color: #27ae60"
            >
            ...
            <td
              v-if="ordenEstado === 'PENDIENTE' && idx === 0"
              :rowspan="items.length"
              style="vertical-align: middle; font-weight: bold; color: #c0392b"
            >
          ...
          <tr v-if="items.length === 0">
            <td
              :colspan="ordenEstado === 'PENDIENTE' ? 11 : 8"
              class="empty-row"
            >
              No hay productos agregados ni órdenes filtradas.
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="footer-venta">
        ...
        <div class="acciones-footer no-print">
          ...
          <!-- 🖨️ Botón de imprimir -->
          <button @click="imprimirFactura" :disabled="!puedeImprimir">
            🖨️ Imprimir
          </button>
          ...
          <div class="pago-container">
            ...
            <div class="pago-input-row">
              ...
              <button
                type="button"
                class="btn-valores-rapidos"
                :disabled="
                  ordenCerrada ||
                  ordenEstado !== 'ABIERTA' ||
                  items.length === 0
                "
                @click.stop="mostrarValoresRapidos = !mostrarValoresRapidos"
                title="Valores rapidos"
              >
                💲
              </button>
            </div>
            ...
            <div
              v-if="mostrarValoresRapidos && !ordenCerrada"
              class="valores-rapidos"
              @click.stop
            >
              <button
                v-for="v in valoresRapidos"
                :key="v"
                @click="seleccionarValorRapido(v)"
              >
                {{ formatPrecioInput(v) }}
              </button>
            </div>
            ...
            <div v-if="descuentoAplicado > 0" class="descuento-linea">
              🏷️ Descuento ({{
                descuentoTipo === "porcentaje"
                  ? descuentoValor + "%"
                  : formatPrecioCOP(descuentoValor)
              }}): -{{ formatPrecioCOP(descuentoAplicado) }}
            </div>
            <div>
              <strong
                >💰 Total a Pagar:
                {{ formatPrecioCOP(calcularTotalFinal) }}</strong
              >
            </div>
            <div>
              💰 Cambio:
              {{
                formatPrecioCOP(Math.max(valorPagado - calcularTotalFinal, 0))
              }}
            </div>
            <button
              type="button"
    ...
    <div v-if="mostrarModalDescuento" class="modal-overlay-descuento">
      <div class="modal-descuento">
        ...
        <div class="desc-tipo">
          <label>
            <input type="radio" v-model="descuentoTipo" value="porcentaje" />
            Porcentaje (%)
          </label>
          <label>
            <input type="radio" v-model="descuentoTipo" value="fijo" /> Valor
            fijo ($)
          </label>
        </div>
        ...
        <div class="desc-input">
          <input
            ...
            min="0"
            :placeholder="
              descuentoTipo === 'porcentaje' ? 'Ej: 10' : 'Ej: 5000'
            "
          />
          <span class="desc-sufijo">{{
            descuentoTipo === "porcentaje" ? "%" : "$"
          }}</span>
        </div>
        ...
        <div class="desc-preview" v-if="descuentoValor > 0">
          ...
          <p>
            <strong
              >Total con descuento:
              {{ formatPrecioCOP(calcularSubtotal - previewDescuento) }}</strong
            >
          </p>
        </div>
        ...
        <div class="desc-botones">
          ...
          <button
            ...
            class="btn-quitar-desc"
            @click="quitarDescuento"
          >
            Quitar Descuento
          </button>
          <button
            class="btn-cancelar-desc"
            @click="mostrarModalDescuento = false"
          >
            Cancelar
          </button>
        </div>
...
} from "@/services/apiUsuariosService.js";
...
export default {
  ...
  computed: {
    tieneDatos() {
      return (
        ...
        (this.cliente.apellidos &&
          String(this.cliente.apellidos).trim() !== "") ||
        ...
        (this.empleado.apellidos &&
          String(this.empleado.apellidos).trim() !== "")
      );
    },
    ...
    totalPagosMixtos() {
      return (
        (Number(this.pagosMixtos.efectivo) || 0) +
  ...
  methods: {
    ...
    cargarEmpleadoDesdeSession() {
      try {
        ...
        if (session?.user) {
          ...
          this.empleado.nombres =
            session.user.nombres || session.user.login || "";
          this.empleado.apellidos = session.user.apellidos || "";
          this.empleadoEncontrado = !!(
            this.empleado.identificacion || this.empleado.nombres
          );
          console.log(
            "👨‍💼 Empleado cargado desde sesión:",
            this.empleado.identificacion,
            this.empleado.nombres,
          );
        }
    ...
    cargarItemsOrdenSeleccionada() {
      const orden = this.ordenesFiltradas.find(
        (o) => o.numeroOrden === this.ordenSeleccionada,
      );
    ...
    setOrdenSeleccionada(orden) {
      ...
      if (orden.descuentoAplicado && orden.descuentoAplicado > 0) {
        this.descuentoTipo =
          (orden.descuentoTipo || "PORCENTAJE") === "PORCENTAJE"
            ? "porcentaje"
            : "fijo";
    ...
    async cargarOrdenesFiltradas() {
      try {
        ...
        if (this.filtrarPorCliente && this.cliente.identificacion) {
          ...
          response = await listarOrdenesPorClienteYEstado(
        ...
        if (rolesUsuario.includes(roleName) && session?.user?.identificacion) {
          ...
          this.ordenesFiltradas = this.ordenesFiltradas.filter(
            (o) => Number(o.identificacionEmpleado) === miId,
          );
        }

        // 🔹 Si es ADMIN y tiene el checkbox "Ver solo mis ventas" activo
        if (
          !rolesUsuario.includes(roleName) &&
          this.filtrarPorMiUsuario &&
          session?.user?.identificacion
        ) {
          const miId = Number(session.user.identificacion);
          this.ordenesFiltradas = this.ordenesFiltradas.filter(
            (o) => Number(o.identificacionEmpleado) === miId,
        ...
        this.mostrarMensaje(
          `✅ ${this.ordenesFiltradas.length} órdenes cargadas.`,
          "success",
        );
    ...
    async buscarClientePorIdentificacionHandler() {
      ...
      try {
        const response = await buscarClientePorIdentificacion(
          Number(this.cliente.identificacion),
        );
        ...
        if (cliente) {
          ...
          this.mostrarMensaje(
            `✅ Cliente encontrado: ${cliente.nombres}`,
            "success",
          );
        } else {
          this.clienteEncontrado = false;
        ...
        this.mostrarMensaje(
          "Error en búsqueda de cliente por identificación.",
          "error",
        );
        this.clienteEncontrado = false;
      }
    ...
    async buscarClientePorNombreHandler() {
      ...
      try {
        ...
        this.mostrarMensaje(
          "Error en búsqueda de cliente por nombre.",
          "error",
        );
        this.clienteEncontrado = false;
      }
    ...
    async buscarEmpleadoPorIdentificacionHandler() {
      ...
      if (!texto) {
        this.mostrarMensaje(
          "Ingrese una identificación del empleado.",
          "error",
        );
        return;
      }
      ...
      try {
        ...
        const tieneCamposUtiles =
          empleadoEncontradoObj &&
          (empleadoEncontradoObj.identificacion ||
            empleadoEncontradoObj.nombres ||
            empleadoEncontradoObj.apellidos);
        ...
        if (tieneCamposUtiles) {
          ...
          this.mostrarMensaje(
            `✅ Empleado encontrado: ${nombres} ${apellidos}`.trim(),
            "success",
          );
        } else {
        ...
        if (
          error.message?.includes("Cannot invoke") ||
          ...
          this.mostrarMensaje(
            error.message || "❌ Error en búsqueda de empleado.",
            "error",
          );
        }
      }
    ...
    async buscarEmpleadoPorNombreHandler() {
      ...
      try {
        ...
        if (tieneCamposUtiles) {
          ...
          this.empleadoEncontrado = false;
          this.mostrarMensaje(
            "⚠️ No se encontraron empleados con ese nombre.",
            "warning",
          );
        }
      } catch (error) {
        ...
        if (
          ...
          this.mostrarMensaje(
            error.message || "❌ Error en búsqueda de empleado por nombre.",
            "error",
          );
        }
      }
    ...
    async buscarProducto() {
      try {
        ...
        if (this.venta.codigo) {
          ...
          this.mostrarMensaje(
            "Ingrese un criterio de búsqueda válido.",
            "error",
          );
          return;
        }
        ...
        this.mostrarMensaje(
          "Error al buscar producto en el servidor.",
          "error",
        );
      }
    },
    ...
    async agregarItem() {
      ...
      if (!this.formValido) {
        this.mostrarMensaje(
          "⚠️ Complete todos los campos antes de agregar.",
          "error",
        );
        return;
      }
      ...
      const idCliente = this.cliente.identificacion
        ? Number(this.cliente.identificacion)
        : 0;
      ...
      const idEmpleado = this.empleado.identificacion
        ? Number(this.empleado.identificacion)
        : 0;
      ...
      try {
        ...
        this.ordenIdentificacionCliente =
          ordenActualizada.identificacionCliente ?? idCliente;
        ...
        this.mostrarMensaje(
          `✅ Producto ${this.venta.producto} agregado correctamente.`,
          "success",
        );
        ...
        this.mostrarMensaje(
          error.message || "Error al agregar producto en el servidor.",
          "error",
        );
      }
    },
    ...
    async eliminarItem(idx) {
      if (idx >= 0 && idx < this.items.length) {
        ...
        if (!this.numeroOrden) {
          this.mostrarMensaje(
            "⚠️ No hay orden abierta asociada. Vuelve a agregar el producto.",
            "error",
          );
          return;
        }
        ...
        try {
          ...
          const response = await restarCantidadProducto(
            this.numeroOrden,
            item.codigo,
            cantidadARestar,
          );
          ...
          if (
            ...
            ordenActualizada.detalles.length > 0
          ) {
            this.items = ordenActualizada.detalles.map((d) => ({
              codigo: d.codigo,
              producto: d.producto,
            ...
            this.items = [];
            this.mostrarMensaje(
              `🗑️ Producto ${item.codigo} eliminado de la orden.`,
              "error",
            );
          }
        } catch (error) {
          ...
          this.mostrarMensaje(
            error.message || "Error al restar producto en el servidor.",
            "error",
          );
        }
      }
    ...
    agregarCliente() {
      ...
      this.cargarEmpleadoDesdeSession();
      this.mostrarMensaje(
        "✅ Listo para registrar una nueva venta.",
        "success",
      );
    },
    ...
    validarAntesDeConfirmar() {
      ...
      if (
        this.totalPagosMixtos < this.calcularTotalFinal &&
        !this.pagosMixtos.comentario.trim()
      ) {
    ...
    async confirmarCerrarVenta() {
      ...
      if (this.pagosMixtos.efectivo > 0) {
        pagos.push({
          metodoPago: "EFECTIVO",
          valor: this.pagosMixtos.efectivo,
          referencia: null,
        });
      }
      if (this.pagosMixtos.tarjetaDebito > 0) {
        pagos.push({
          ...
          valor: this.pagosMixtos.tarjetaDebito,
          referencia: this.pagosMixtos.referencia || null,
        });
      }
      if (this.pagosMixtos.tarjetaCredito > 0) {
        pagos.push({
          metodoPago: "TARJETA_CREDITO",
          valor: this.pagosMixtos.tarjetaCredito,
          referencia: this.pagosMixtos.referencia || null,
        });
      }
      if (this.pagosMixtos.transferencia > 0) {
        pagos.push({
          ...
          valor: this.pagosMixtos.transferencia,
          referencia: this.pagosMixtos.referencia || null,
        });
      }
      ...
      const payload = {
        ...
        descuentoAplicado:
          this.descuentoAplicado > 0 ? this.descuentoAplicado : null,
        pagos: pagos,
      ...
      try {
        ...
        this.mostrarMensaje(
          "✅ Venta cerrada correctamente con " +
            pagos.length +
            " método(s) de pago.",
          "success",
        );
        ...
        this.mostrarMensaje(
          error.message || "Error al cerrar la venta en el servidor.",
          "error",
        );
      }
    },
    ...
    aplicarDescuento() {
      if (this.descuentoTipo === "porcentaje") {
        this.descuentoAplicado = Math.round(
          (this.calcularSubtotal * this.descuentoValor) / 100,
        );
      } else {
    ...
    abrirModalEliminarOrden() {
      if (!this.numeroOrden) {
        this.mostrarMensaje(
          "⚠️ No hay una orden valida para eliminar.",
          "warning",
        );
        return;
      }
...
.btn-valores-rapidos:hover {
  background: #f0f0f0;
}
.btn-valores-rapidos:disabled {
  opacity: 0.4;
  cursor: not-allowed;
}
