  ...
  methods: {
    ...
    async agregarProveedor() {
      ...
      const existente = this.proveedores.find(
        (p) => p.codigoSucursal === Number(this.proveedorForm.codigoSucursal),
      );
      if (existente) {
        this.mostrarMensaje(
          `⚠️ Ya existe un proveedor: ${existente.nombre} con este Código de Sucursal (${existente.codigoSucursal}).`,
          "error",
        );
        return;
      }
      ...
      try {
        ...
        this.manejarErrorApiProveedorRegistrar(
          error,
          `registrar proveedor ${this.proveedorForm.nombre}`,
        );
      }
    },
